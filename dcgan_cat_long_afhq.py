...
def main():
    ...
    _output_dir = "results_cats_dcgan_long_afhq"
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for batch_idx, (real, _) in enumerate(loader):
            real = real.to(device)
            if epoch < NUM_EPOCHS * 0.1: # Apenas nos primeiros 10% do treino
                # real = real + (0.1 * torch.randn_like(real))
                real.add_(0.1 * torch.randn_like(real))
